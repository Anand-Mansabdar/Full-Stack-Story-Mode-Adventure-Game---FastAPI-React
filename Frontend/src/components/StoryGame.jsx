  ...
  return (
    <div className="animate-fade-in-up">
      ...
      <div className="text-center mb-10">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-sm font-medium mb-4">
      ...
      {/* Story content card */}
      <div
        className={`max-w-2xl mx-auto transition-all duration-300 ${transitioning ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"}`}
      >
        {currentNode && (
          <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl rounded-2xl border border-slate-200/60 dark:border-slate-700/60 shadow-xl shadow-slate-200/50 dark:shadow-slate-900/50 overflow-hidden">
            ...
            {isEnding ? (
              <div
                className={`p-8 md:p-10 border-t ${isWinningEnding ? "border-emerald-200 dark:border-emerald-800/40 bg-linear-to-br from-emerald-50/80 to-teal-50/80 dark:from-emerald-950/30 dark:to-teal-950/30" : "border-slate-200 dark:border-slate-700 bg-linear-to-br from-slate-50/80 to-slate-100/80 dark:from-slate-800/50 dark:to-slate-900/50"}`}
              >
                <div className="text-center">
                  {/* Icon */}
                  <div
                    className={`w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center ${isWinningEnding ? "bg-emerald-100 dark:bg-emerald-900/40" : "bg-slate-200 dark:bg-slate-700"}`}
                  >
                    {isWinningEnding ? (
                      <svg
                  ...
                  <h3
                    className={`font-display text-2xl font-bold mb-2 ${isWinningEnding ? "text-emerald-700 dark:text-emerald-300" : "text-slate-700 dark:text-slate-200"}`}
                  >
                    {isWinningEnding ? "🎉 Congratulations!" : "The End"}
                  </h3>

                  <p
                    className={`text-base mb-6 ${isWinningEnding ? "text-emerald-600 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`}
                  >
                    {isWinningEnding
                ...
                <div className="space-y-3">
                  {options.map((option, index) => (
                    <button
                      ...
                      key={index}
                      id={`option-btn-${index}`}
                      className="w-full text-left px-5 py-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-700 dark:text-slate-200 hover:border-slate-300 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/50 hover:text-slate-800 dark:hover:text-slate-100 transition-all duration-200 cursor-pointer group flex items-center gap-3"
                    >
                      <span className="shrink-0 w-8 h-8 rounded-lg bg-slate-100 dark:bg-slate-700 group-hover:bg-slate-200 dark:group-hover:bg-slate-600 flex items-center justify-center text-sm font-bold text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors duration-200">
                      ...
                      <svg
                        className="w-5 h-5 ml-auto shrink-0 text-slate-300 dark:text-slate-600 group-hover:text-slate-500 dark:group-hover:text-slate-400 transition-colors duration-200"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
